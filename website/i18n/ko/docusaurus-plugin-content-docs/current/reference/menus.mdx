...
```go title="Package: github.com/frankfang/wails/v2/pkg/menu"
type Menu struct {
    Items []*MenuItem
}
```
...
Accelerators (sometimes called keyboard shortcuts) define a binding between a keystroke and a menu item. Wails defines an Accelerator as a combination or key + [Modifier](#modifier). They are available in the `"github.com/frankfang/wails/v2/pkg/menu/keys"` package.

Example:

```go title="Package: github.com/frankfang/wails/v2/pkg/menu/keys"
